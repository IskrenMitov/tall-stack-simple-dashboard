            ...
            <table class="w-full text-sm text-left text-gray-500 rounded">
                ...
                @foreach($authors as $author)
                    <tr class="border-b bg-gray-800 border-gray-700">
                        <td scope="row" class="px-6 py-4 font-body">{{ $author->name }}</td>
                        <td class="font-body">{{ $author->age }}</td>
                        <td>
                            <a href="{{ route('authors.show', $author->id) }}" class="btn-link hover:underline inline-flex items-center">
